...
public class StorePostsApiController {
    ...
    @ApiOperation(
            value = "제품 게시글 등록",
            notes = "판매자가 제품을 등록하기 위해 사용합니다. <br>" +
                    "Response : 저장된 StorePost의 Id",
            code = 201)
    @ApiResponses({
    ...
    @ApiOperation(
            ...
            notes = "제품 게시글을 등록하기 전에 image 를 등록합니다.<br>" +
                    ...
                    "해당 값으로 얻은 id를 게시글 등록에 사용하지 않으면 서버에 파일이 저장되게 됩니다. 따라서 반드시 게시글 등록에 사용해주세요 <br>" +
                    "Response : 저장된 image의 id, (content image, preview image 순서로)",
            code = 200)
    ...
    @ApiOperation(
            value = "제품 게시글 수정",
            notes = "판매자 혹은 관리자가 게시글 수정에 사용합니다. <br>" +
                    "Response : Nothing",
            code = 200)
    @ApiResponse(code = 500, message = "server error")
    @ApiImplicitParams({
    ...
    @ApiOperation(
            ...
            notes = "게시글에 대한 모든 내용을 가져옵니다. (title, content, 작성자, 수정일시, 수정자 등)" +
                    "Response : StorePostsResponseDto",
            code = 200)
    @ApiResponse(code = 500, message = "server error")
    @ApiImplicitParam(name = "id", value = "제품 게시글의 id (제품 id X)")
    @ResponseStatus(HttpStatus.OK)
    ...
    @ApiOperation(
            ...
            notes = "게시글을 조회합니다. 연관된 product를 모두 가져옵니다 <br>" +
                    "Response : StorePostWithProductDto",
            code = 200
    )
    @ResponseStatus(HttpStatus.OK)
